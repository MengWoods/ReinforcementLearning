...
LOGPATH = 'logtest/ddpg'
NOTE = 'ddpg-noise free'
...
#        tf.summary.histogram('action',action)
    ...
    def actorNetwork(self):       
        ...
        w_init = tflearn.initializations.uniform(minval=-0.003, maxval=0.003) 
        out = tflearn.fully_connected(net_2, self.a_dim, activation='tanh', weights_init=w_init) # tanh from -1 to 1
...
#      Train 
# ===================================    
def train(sess, env, module, max_ep, max_step, logpath, buffer, render, actor_noise,stateNoiseLevel):
    ...
    for i in range(max_ep):
        s = env.reset() 
        s = state_noise(s, stateNoiseLevel)
        ep_reward = 0
        ep_ave_max_q = 0
        ...
        for j in range(max_step):
            ...
            a = module.a_predict(np.reshape(s, (1, module.s_dim))) + actor_noise()
            s2, r, terminal, info = env.step(a[0])
            s2 = state_noise(s2, stateNoiseLevel)
            buffer.add(np.reshape(s, (module.s_dim,)), np.reshape(a, (module.a_dim,)), r, terminal, np.reshape(s2, (module.s_dim,)))
...
def main():
    with tf.Session() as sess:
        ...
        train(sess, env, module, max_ep=MAX_EPISODES, max_step=MAX_EP_STEPS, logpath=LOGPATH,
              buffer=buffer, render=RENDER, actor_noise=actor_noise, stateNoiseLevel=NOISE_LEVEL,
              )
